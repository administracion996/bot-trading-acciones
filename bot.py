...
def get_gemini_decision(ticker: str, price: float, rsi: float, vol: float) -> str:
    prompt = f"""
    Actúa como gestor cuantitativo. Analiza las siguientes métricas del activo {ticker}:
...
def run_trading_bot():
    ...
    for ticker in TICKERS:
        ...
        print(f"[{ticker}] Precio: ${current_price_usd:.2f} | RSI: {current_rsi:.1f} | Decision: {signal}")

        # 1. EVALUAR VENTA (CON CANDADO DE SEGURIDAD ESTRICTO)
        if signal == "SELL" and ticker in posiciones:
            ...
            if beneficio_neto_eur > 0.00:
                ...
                print(f"✅ VENTA EJECUTADA: {ticker} | Beneficio: +{beneficio_neto_eur:.2f} €")
            else:
                print(f"⛔ VENTA BLOQUEADA: {ticker} | Beneficio ({beneficio_neto_eur:.2f} €) <= 0.00 €")

        # 2. EVALUAR COMPRA
        elif signal == "BUY" and ticker not in posiciones:
            capital_asignado_eur = efectivo_eur * MAX_CAPITAL_PER_TRADE_PCT
            if capital_asignado_eur > 50.0:
                capital_asignado_usd = (capital_asignado_eur - COMISION_BROKER_EUR) * eurusd
                cantidad = capital_asignado_usd / current_price_usd
                ...
                print(f"🛒 COMPRA EJECUTADA: {ticker} | Inversion: {coste_total_eur:.2f} €")
